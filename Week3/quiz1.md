...
- 답: Working directory, Staging area, Repo
...
- 답: 
    ...
    1은 새로 만든 branch의 모든 커밋 메세지들을 저장하기 위해 커밋 메세지의 개수 만큼 base branch에 새로운 커밋을 만들고 그 곳에 커밋 메세지들을 저장하는 것. 

    2는 새로 만든 branch의 모든 커밋들을 하나로 모아서 base branch에 있던 기존 commit에 욱여넣는 것.
    
    3은 새로 만든 branch에 있는 커밋들을 base branch에 옮겨버리는 것.
...
- 답: revert. reset은 commit id를 바꾸므로 문제가 생길 수 있기 때문이다.